...
export const initializeAllData = async () => {
  try {
    ...
    // Check if we've initialized recently (within the last 60 seconds)
    const lastInitTime = localStorage.getItem('lastInitTimestamp');
    if (lastInitTime) {
      const now = new Date().getTime();
      const timestamp = parseInt(lastInitTime, 10);
      const sixtySeconds = 60 * 1000;

      if (now - timestamp < sixtySeconds) {
        console.log('Using cached data (initialized less than 60 seconds ago)');
    ...
    try {
      ...
      const promises = [
        fetchFinancialProfile().catch(err => {
          console.warn('Error fetching financial profile:', err);
        ...
        calculateCreditScore({}).catch(err => {
          console.warn('Error calculating credit score:', err);
          return null; // Return null instead of rejecting
        })
      ];
      ...
      localStorage.setItem('lastInitTimestamp', new Date().getTime().toString());

      console.log('All data initialized successfully in parallel');
      return true;
    } catch (parallelError) {
      console.error('Error in parallel data initialization:', parallelError);

      // Fall back to sequential synchronization if parallel fails
      console.log('Falling back to sequential synchronization...');
      const result = await synchronizeData();

      // Store the initialization timestamp
      localStorage.setItem('lastInitTimestamp', new Date().getTime().toString());

      console.log('All data initialized successfully (sequential fallback):', result);
      return true;
    }
  } catch (error) {

...
/**
 * A basic batch processor for generating multiple reports
 * This component is completely self-contained and doesn't rely on parent functions
 */
const BasicBatchProcessor = ({ onComplete, onCancel }) => {
  ...
  const processEmail = async (email) => {
    try {
      ...
      const reportUrl = reportResponse.data.report_url;

      try {
        // Create a hidden anchor element to trigger the download
        const downloadLink = document.createElement('a');
        downloadLink.href = `http://127.0.0.1:8000${reportUrl}`;
        downloadLink.target = '_blank';
        downloadLink.download = `credit_report_${user.email.replace('@', '_at_')}.pdf`;

        // Append to the document, click it, and remove it
        document.body.appendChild(downloadLink);
        downloadLink.click();

        // Give the browser a moment to start the download before removing the element
        setTimeout(() => {
          document.body.removeChild(downloadLink);
        }, 100);

        console.log(`Download initiated for ${email}`);
      } catch (downloadErr) {
        console.error(`Error downloading report for ${email}:`, downloadErr);
      }

      // Step 4: Add successful result
      addResult({
        email,
        status: 'success',
        reportUrl: reportUrl,
      ...
      // Update reports generated count in localStorage directly
      try {
        const currentCount = parseInt(localStorage.getItem('adminReportsGeneratedCount') || '0', 10);
        localStorage.setItem('adminReportsGeneratedCount', (currentCount + 1).toString());
      } catch (err) {
  ...
  const addResult = (result) => {
    ...
    // Move to next email or finish with a longer delay to ensure downloads complete
    setTimeout(() => {
    ...
    }, 1500); // Increased delay to ensure downloads have time to start
  };
...
export default BasicBatchProcessor;

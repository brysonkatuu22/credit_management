...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    // Load cached data immediately to prevent loading screens
    try {
      // 1. Get user info from localStorage first for immediate display
      ...
      // 2. Get credit score from localStorage
      ...
      // 3. Get dashboard metrics from localStorage
      const cachedMetricsJson = localStorage.getItem('dashboardMetrics');
      if (cachedMetricsJson) {
        const cachedMetrics = JSON.parse(cachedMetricsJson);
    ...
    // Use a small timeout to allow the UI to render first with cached data
    setTimeout(() => {
                ...
                <div className="d-flex justify-content-between align-items-center">
                  <div>
                    ...
                    <div className="text-xl font-bold text-white">
                      {creditScore ?
                        (creditScore.fallback || creditScore.cached ?
                          'Not calculated yet' :
                          creditScore.score) :
                        'Not calculated yet'}
                    </div>
                    <div className="text-xs text-white opacity-75 mt-1">
                      {creditScore && !creditScore.fallback && !creditScore.cached ?
                        'Your latest calculated score' :
